...
| 循迹输入 | 8路红外GPIO、7路数字GPIO、7路模拟ADC | Gmr八路红外、Full七路数字灰度 | Gmr的IR8使用PB9；模拟采样保留 |
| 循迹算法 | 关闭、数字量加权 | Gmr和Full均为数字量加权 | Gmr用于Task2/6/7，Full用于Task1/Task4 |
| 循迹输出 | 直接PWM、PWM+编码器交叉同步、编码器速度闭环 | 编码器速度闭环 | 三种组合均通过编译；参数独立 |
| 直角转向 | 关闭、单轮锁死、双轮反转 | 两Profile均关闭 | 当前任务只运行普通加权差速循迹 |
| 强转速度 | 固定速度、JY61转角线性曲线 | 固定速度占位 | 直角库关闭，不参与当前输出 |
...
| H7 LCD显示 | 关闭、UART文本行协议 | Gmr开启、Full关闭 | Gmr使用UART2，支持0～9行增量刷新和任务命令 |
| H7 IMU输入 | 关闭、UART JY61兼容帧 | GMR关闭、Full开启 | 与板载UART1 JY61独立 |
| 天猛星SPI六轴 | 关闭、自动识别、IMU660RA/RB/RC | 两Profile均关闭 | Gmr的IR8占用PB9/SCK，禁止同时启用 |
...
| 任务 | 必需库 |
| --- | --- |
| Task1 | 循迹算法；关闭直角转向时持续运行到人工停止 |
| Task2/Task3/Task7 | 二维视觉云台 |
| Task4 | 循迹算法 + 二维视觉云台；双IMU可单独关闭 |
...
该组合保留Task1/Task4和`MotorNoYaw_ApplyTask1LineCommand()`；任务不再按转向次数
自动完成，持续运行到人工停止。
...
MSPM0G3507可用Flash为128 KiB。2026-07-29用TI Arm Clang 4.0.4 LTS、`-O2`构建：

| 板型 | Flash text | 占用 | 剩余 |
| --- | ---: | ---: | ---: |
| 天猛星Gmr赛题控制台 | 38552 B | 29.4% | 92520 B |
| 天猛星Full | 90912 B | 69.4% | 40160 B |
