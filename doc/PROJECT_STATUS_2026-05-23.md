...
- `app/`：应用层逻辑，包括菜单、状态机、循迹、速度命令和任务框架。
...
- 四个闭环步进电机已改成 `STEP/DIR` 控制框架，STEP 输出迁到 TIMG0 20us 定时器中断，不再由主循环补发脉冲。
- 灰度传感器当前默认按数字量模块处理，MCU 直接读 PA15/PA16/PA17/PA24/PA25/PA26/PA27 的 GPIO 电平；ADC 阈值逻辑只作为模拟灰度备用。
- 外部编码器和 MCU 速度闭环代码已删除；路线距离直接读底盘左右 STEP 输出计数。
...
路程统计直接读取 `hardware/stepper_pulse.c` 里由 TIMG0 中断累计的 STEP 数。路线层取底盘左右电机 STEP 相对值的平均值，后续实车只需要标定“多少 STEP 对应多少距离”。
...
关闭后 `LOG_*` 宏为空操作，启动、按键、状态机、路线阶段、循迹异常等行为日志不再输出。PA10/PA11 仍作为 Type-C/BSL 保留脚，不建议复用给其它外设。
...
- STEP 计数表示 MCU 已输出的脉冲数，不等价于驱动器确认到位；若后续需要真实闭环确认，再接步进驱动器反馈或报警输入。
