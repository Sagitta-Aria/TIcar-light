...
本文按地猛星 MSPM0G3507 最小系统板 H3/H5 排针、板载外设和 1.1ccs 代码配置记录接线。没有写进“当前主控接线”的自由口，先不要随手接外设，避免后续灰度、UART 或下载恢复互相抢脚。
...
当前 `hardware/motor.c` 会在 `Motor_Task()` 里按命令发 STEP 脉冲；方向反了先在接线表里记录，再决定是否加单电机方向反相配置。
